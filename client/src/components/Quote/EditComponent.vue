...
export default {
  ...
  created() {
    let uri = `/quotes/edit/${this.$route.params.id}`;
    if (process.env.NODE_ENV !== 'production') {
      uri = `http://localhost:4000/quotes/edit/${this.$route.params.id}`;
    }
  ...
  methods: {
    updateQuote() {
      let uri = `/quotes/update/${this.$route.params.id}`;
      if (process.env.NODE_ENV !== 'production') {
        uri = `http://localhost:4000/quotes/update/${this.$route.params.id}`;
      }
